...
import { useSearchStore, useSearchHydration, useSearchActions } from '@/stores/customer/search-store';
...
export default function SearchScreen() {
  ...
  const { searchMode, setSearchMode, setSearchQuery: setStoreQuery, handleFiltersChange, filters } = useSearchStore();
  const { setUserLocation, clearUserLocation } = useSearchActions();
  ...
  const { hasPermission, isLoading: locationLoading, requestPermission, getCurrentLocation } = useLocationPermission();
  ...
  const ServiceItem = React.memo(({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity 
        activeOpacity={0.7} 
        className="mb-2"
        onPress={() => router.push(`/(customer)/service/${item.id}` as any)}
      >
  ...
  const ProviderItem = React.memo(({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity activeOpacity={0.7} className="mb-2" onPress={() => router.push(`/(customer)/provider/${item.id}` as any)}>
        <Card className=" border-border/50 bg-card">
          <CardContent className="p-3">
            <View className="flex-row items-start gap-2.5">
              {/* Provider Avatar */}
                    ...
                    <View className="flex-row items-center gap-0.5">
                      ...
                      <Text className="text-xs text-muted-foreground" numberOfLines={1}>
                        {item.distance !== null && item.distance !== undefined
                          ? `${item.distance.toFixed(1)} miles • ${item.city && item.country ? `${item.city}, ${item.country}` : 'Location not specified'}`
          ...
          <View className="flex-row items-center justify-between">
            ...
            <View className="flex-row items-center gap-2">
              ...
              <TouchableOpacity
                ...
                disabled={locationLoading || isGettingLocation}
              ...
              >
                <Ionicons
                  name={filters.userLatitude && filters.userLongitude ? "location" : "location-outline"}
                  size={12}
                  color={
                    filters.userLatitude && filters.userLongitude
                      ? (isDarkColorScheme ? THEME.dark.success : THEME.light.success)
                      : (isDarkColorScheme ? THEME.dark.mutedForeground : THEME.light.mutedForeground)
                  }
                />
                <Text className={`text-xs font-medium ${
                  ...
                  {isGettingLocation ? 'Getting...' : (filters.userLatitude && filters.userLongitude ? 'Located' : 'Location')}
                </Text>
              </TouchableOpacity>
